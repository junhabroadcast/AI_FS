...
- 통계: 평균·중앙 ROI·p05/p95 Y, 하이라이트/섀도우 비율, 왜도
- 경량 AI 스코어: 피처 로지스틱 → `tanh` 노출 점수 (-1~+1)
- 규칙 + AI 융합, EMA로 라벨 깜빡임 억제
- 라벨: DARK / NORMAL / BRIGHT / OVER / UNDER / BLACK (한글 상태바)
- 고속 경로: `judge_bgr_fast` (축소 BGR + 근사 Y), 목표 **30 fps**
- OpenCV 경량 오버레이 (라벨·Y%·score 바)

## GUI — AiFsMonitor

- Device 콤보 + Refresh / Start / Stop (WFM UX)
- 실시간 프리뷰 + 하단 상태바 (`LOCKED` / `NO SIGNAL` / 판정 / fps)

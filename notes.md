...
v1.1.0은 **DeckLink SDI 실시간 기준 대비 밝기·색 판정 GUI/exe**와, FS 자동화 가능성을 보여주는 **소프트웨어 데모**(싱크·색교정·QC)를 포함합니다.

포함:

- AiFsMonitor (장치 선택, Start/Stop, **기준 캡처**, 30fps 판정)
- DeckLink COM 캡처 (MTA 워커)
- 기준 프레임 대비 ΔY / ΔC 판정 (`reference_judge`)
- 절대 밝기 엔진 (CLI·배치 데모용)
...
SDI → DeckLink COM (MTA thread)
    → UYVY/v210 → BGR
    → (기준 캡처) RefFeatures 저장
    → ReferenceJudge (ΔY / ΔC + EMA)
    → Tk 프리뷰 + 상태바
...
| `ai_fs/reference_judge.py` | 기준 대비 밝기·색 판정 |
| `ai_fs/brightness.py` | 절대 밝기 피처·규칙·AI 스코어·EMA |
...
4. Start 후 **기준 캡처**를 해야 판정이 시작됩니다. 기준이 없으면 안내만 표시됩니다.
5. OpenCV 5: UYVY는 `(H, W, 2)` shape여야 합니다. (v1.0.0에서 수정)
6. Demo 장치로 UI/판정만 먼저 확인한 뒤 SDI로 전환하세요.
7. Qt 경로는 WFM과 무관합니다. AI FS GUI는 Tkinter입니다.

## 성능

- 목표 표시·판정: **30 fps**
- 판정은 폭 160px 근사 Y/Cb/Cr 통계 (~1 ms)
- 캡처 표시 상한 약 854px 폭
...
- 기준 대비·밝기 고속 경로는 Rec.709 근사 휘도 가중
